...
import './MotorcycleForm.css'; // Ensure you have a corresponding CSS file
...
const MotorcycleForm = () => {
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setFormData({
            ...formData,
            photos: files // Handle motorcycle photos
        });
    ...
    const handleConfirm = async () => {
        ...
        try {
            const docRef = await addDoc(collection(db, 'motorcycles'), {
                ...
                category: 'Bikes / Motorcycles', // Explicitly set the category as Bikes
            });

            console.log('Document written with ID: ', docRef.id);
            resetForm(); // Reset the form after successful submission
            setIsReview(false); // Return to form state
            alert('Motorcycle details submitted successfully!');
        } catch (error) {
            console.error('Error adding document: ', error);
            alert('Error submitting the motorcycle details. Please try again.');
        } finally {
    ...
    return (
        <div className="motorcycle-form-container">
            ...
            <header className="form-header">
                <h2>POST YOUR MOTORCYCLE AD</h2>
                <div className="category-selector">
                    <span>Selected Category: Bikes / Motorcycles</span>
                </div>
            </header>
