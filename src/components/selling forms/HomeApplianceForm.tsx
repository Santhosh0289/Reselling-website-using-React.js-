...
const HomeApplianceForm = () => {
    const [formData, setFormData] = useState({
        ...
        price: '',
        location: '',
        category: 'Electronics & Appliances / Home Appliances', // Set the category here
        adTitle: '',
        description: '',
    ...
    const resetForm = () => {
        setFormData({
            ...
            price: '',
            location: '',
            category: 'Electronics & Appliances / Home Appliances', // Ensure correct category reset
            adTitle: '',
            description: '',
    ...
    const handleConfirm = async () => {
        ...
        try {
            const docRef = await addDoc(collection(db, 'home_appliances'), {
            ...
            resetForm();
            setIsReview(false);
            alert('Home appliance details submitted successfully!');
        } catch (error) {
            console.error('Error adding document: ', error);
            alert('Error submitting the home appliance details. Please try again.');
        } finally {
    ...
    return (
        <div className="home-appliance-form-container">
            {/* Header Section */}
            <header className="form-header">
                <h2>POST YOUR HOME APPLIANCE AD</h2>
                <div className="category-selector">
                    <span>Selected Category: Electronics & Appliances / Home Appliances</span>
                </div>
            </header>

            {!isReview ? (
                <form onSubmit={handleSubmit} className="home-appliance-form">
