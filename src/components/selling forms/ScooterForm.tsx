...
import './ScooterForm.css'; // Ensure you have a corresponding CSS file
...
const ScooterForm = () => {
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setFormData({
            ...formData,
            photos: files // Handle scooter photos
        });
    ...
    const handleConfirm = async () => {
        ...
        try {
            const docRef = await addDoc(collection(db, 'scooters'), {
                ...
                category: 'Bikes / Scooter', // Explicitly set the category as Bikes
            });

            console.log('Document written with ID: ', docRef.id);
            resetForm(); // Reset the form after successful submission
            setIsReview(false); // Return to form state
            alert('Scooter details submitted successfully!');
        } catch (error) {
            console.error('Error adding document: ', error);
            alert('Error submitting the scooter details. Please try again.');
        } finally {
    ...
    return (
        <div className="scooter-form-container">
            {/* Header Section */}
            <header className="form-header">
                <h2>POST YOUR SCOOTER AD</h2>
                <div className="category-selector">
                    <span>Selected Category: Bikes / Scooters</span>
                </div>
            </header>
